...
func PublishInTx(
	...
	decorator := NewTracingPublisherDecorator(publisher)

	return decorator.Publish("ItemAddedToCart", msg)
}
...
func (tpd TracingPublisherDecorator) Publish(topic string, messages ...*message.Message) error {
	for i, _ := range messages {
		otel.GetTextMapPropagator().Inject(messages[i].Context(), propagation.MapCarrier(messages[i].Metadata))
	}
	return tpd.pub.Publish(topic, messages...)
}

func (tpd TracingPublisherDecorator) Close() error {
	return tpd.Close()
}
